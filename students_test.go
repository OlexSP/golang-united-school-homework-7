...
func TestPeople_Swap(t *testing.T) { // had changes to meet the Independence principle.
	var PList People = []Person{
		{
			firstName: "Ivan",
			lastName:  "Drago",
			birthDay:  time.Date(1985, time.May, 25, 0, 0, 0, 0, time.UTC),
	...
	PList.Swap(tData.FirstPeopleInd, tData.SecondPeopleInd)
	got := PList[tData.SecondPeopleInd]
...
func TestMatrix_Set(t *testing.T) {
	...
	tData := map[string]struct {
		...
		"RowLessO":       {-1, 1, 5, needMatrixF, false},
